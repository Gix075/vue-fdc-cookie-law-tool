<template>
    <div v-if="banner == true && tool.banner.show == true" class="fdc-banner" :class="tool.banner.class">
        <div class="fdc-banner--inner">
            <div v-if="tool.banner.buttons.close.active" class="fdc-banner--close">x</div>
    ...
    <div v-if="page === 'cookies'" class="fdc-policypage">
        <h1>Cookie Policy</h1>
        <div class="fdc--choises-panel">
            ...
            <div v-for="servicecat in tool.config_cookie_policy.services" :key="servicecat.catName" class="fdc--choises-panel--services-category">
                ...
                <div v-for="(service,index) in servicecat.services" :key="index" :data-service="index" class="fdc--choises-panel--service">
                    <div class="fdc--choises-panel--service-text" v-html="tool.docs.cookie_policy_docs[index]"></div>
                    <div class="fdc--choises-panel--service-buttons" :class="isSingeServiceAccepted(index,true)">
                        <span class="fdc-button-wrapper button--accept">
                            <button class="fdc-button button--accept" @click="handleSingleServiceChoise(index,true)">Accetto</button>
                        </span>
                        <span class="fdc-button-wrapper button--reject">
                            <button class="fdc-button button--reject" @click="handleSingleServiceChoise(index,false)">Rifiuto</button>
                        </span>
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            isSingeServiceAccepted(service,return_class=false) {
                let is_accepted = new fdcCookieLawTool(this.tool.config,this.tool.docs).checkSingleService(service);
                if ( return_class === true && is_accepted != undefined ) {
                    return ( is_accepted === true ) ? "service--accepted" : "service-not-accepted";
                }
                else if ( return_class === true && is_accepted === undefined )  {
            ...
            handleSingleServiceChoise(service,action) {
                ...
                fdc.handleSingleServiceChoise(service,action);
                this.isSingeServiceAccepted(service,true);
            },
...
<style>
    ...
    .fdc--choises-panel--services-category > h3 {
        font-size: 2em;
        margin: 0 0 20px 0;
    }
    ...
    .fdc--choises-panel--service-buttons.service--accepted .button--accept {
        opacity: 0.5;
        pointer-events: none;
    }

    .fdc--choises-panel--service-buttons.service-not-accepted .button--reject {
        opacity: 0.5;
        pointer-events: none;
    }

    .fdc--choises-panel--service-text p,
    .fdc--choises-panel--service-text h5,
    .fdc--choises-panel--service-text ul {
        display: none;
    }
    ...
    .fdc-banner--text--content p {
        margin-top: 0;
    }

    .fdc-banner.banner--bottom {
        bottom: 20px;
        top: auto;
    }
    ...
    .fdc-button:hover,
    button.fdc-button:hover,
    a.fdc-button:hover {
        opacity: 0.8;
    }

    .fdc-button.button--accept,
    button.fdc-button.button--accept,
    a.fdc-button.button--accept {
        background-color: #42b983;
        border-color: #42b983;
    }
    

    .fdc-button.button--reject,
    button.fdc-button.button--reject,
    a.fdc-button.button--reject {
        background-color: #f02d3a;
        border-color: #f02d3a;
    }

    .fdc-button.button--customize,
    button.fdc-button.button--customize,
    a.fdc-button.button--customize {
